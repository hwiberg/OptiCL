...
def optimization_MIP(model,
    ...
    def constraints_linear(model, outcome, coefficients, lb=None, ub=None, weight_objective=0):
        ...
        'Coefficients' is a model file generated by the constraint_extrapolation_skEN() or constraint_extrapolation_skSVM() function.
...
def check_model_master(model_master, print_model = True):
    ...
    for i, row in model_master.iterrows():
        ...
        assert row['model_type'] in ['iai','rf','cart','linear','svm','mlp', 'gbm'], "Invalid model type selected."
        if row['objective'] != 0:
