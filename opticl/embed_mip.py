...
def optimization_MIP(model,
    ...
    def constraints_linear(model, outcome, task, coefficients, lb=None, ub=None, weight_objective=0):
        '''
    ...
    def constraints_svm(model, outcome, task, coefficients, lb=None, ub=None, weight_objective=0):
        '''
        ...
        model.add_component('SVM'+outcome, Constraint(expr=model.y[outcome] == sum(model.x[i] * coeff.loc[i] for i in N) + intercept))

        # Set y to binary: 1 if expr >= 0, else 0
        # model.add_component('SVM_lb'+outcome, Constraint(expr=model.y[outcome] >= 1/M*(sum(model.x[i] * coeff.loc[i] for i in N) + intercept)))
    ...
    def constraints_gbm(model, outcome, task, gbm_table, ub=None, lb=None, weight_objective=0):
        '''
    ...
    def constraints_mlp(model, outcome, task, weights, lb=None, ub=None, weight_objective=0, M_l=-1e5, M_u=1e5):
        '''
        ...
        v_input = [x[i] for i in X.columns]
    ...
    for i, row in model_master.iterrows():
        ...
        if mtype in ['cart', 'iai', 'iai-single']:
            ...
            constraints_gbm(model, row['outcome'], row['task'], mfile,
                lb=row['lb'], ub=row['ub'], weight_objective=row['objective'])
        elif mtype == 'mlp':
            constraints_mlp(model, row['outcome'], row['task'], mfile,
                lb=row['lb'], ub=row['ub'], 
                weight_objective=row['objective'])
