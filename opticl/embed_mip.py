...
def optimization_MIP(model, model_master):
    ...
    # def constraints_svm(model, outcome, task, coefficients, var_features, contex_features, lb=None, ub=None, weight_objective=0, SCM=None, features=None):
    #     '''
            ...
            if enlarge[0] == 1:
                ...
                def constraint_CTR1(model, i):
                    if i in var_features:
                        ...
                        return contex_features[i] == sum(model.lam[outcome, k] * data.loc[k, i] for k in samples)
                model.add_component(outcome+'ConstraintClusteredTrustRegion1', Constraint(rule=sum(model.lam[outcome, k] for k in samples) == 1))
    ...
    for i, row in model_master.iterrows():
        ...
        if row['trust_region']:
            ...
            constraints_tr(model, row['outcome'], data, var_features, contex_features, clustering_model, row['enlargement'])
        if row['objective']!=0:
        ...
        if mtype in ['cart', 'iai', 'iai-single']:
            constraints_tree(model, row['outcome'], mfile, var_features, contex_features,
                lb=row['lb'], ub=row['ub'],
                weight_objective=row['objective'], SCM=row['SCM_counterfactuals'], features=row['features'])
        elif mtype == 'rf':
            constraints_rf(model, row['outcome'], mfile, var_features, contex_features,
                lb=row['lb'], ub=row['ub'],
                max_violation=max_violation, ## additional argument required for RF
                weight_objective=row['objective'], SCM=row['SCM_counterfactuals'], features=row['features'])
        elif mtype == 'gbm':
            constraints_gbm(model, row['outcome'], row['task'], mfile, var_features, contex_features,
                lb=row['lb'], ub=row['ub'], weight_objective=row['objective'], SCM=row['SCM_counterfactuals'],
                            features=row['features'])
        elif mtype == 'mlp':
            constraints_mlp(model, data, row['outcome'], row['task'], mfile, var_features, contex_features,
                lb=row['lb'], ub=row['ub'],
                weight_objective=row['objective'], SCM=row['SCM_counterfactuals'], features=row['features'])
        elif mtype in ['linear', 'svm']:
            constraints_linear(model, row['outcome'], row['task'], mfile, var_features, contex_features,
                lb=row['lb'], ub=row['ub'],
                weight_objective=row['objective'], SCM=row['SCM_counterfactuals'], features=row['features'])
        return model
