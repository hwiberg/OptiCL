...
def optimization_MIP(model, model_master):
    ...
    def constraints_tr(model, outcome, data, clustering_model):
        '''
        ...
        if clustering_model is not None:
            ...
            model.add_component(outcome + 'ConstraintClusteredTrustRegion1', Constraint(data.columns, rule=constraint_CTR1))
            model.add_component(outcome + 'ConstraintClusteredTrustRegion2', Constraint(np.unique(clustering_model.labels_), rule=constraint_CTR2))
            model.add_component(outcome + 'ConstraintClusteredTrustRegion3', Constraint(rule=sum(model.u[label] for label in np.unique(clustering_model.labels_)) == 1))
        else:
    ...
    for i, row in model_master.iterrows():
        ...
        if row['trust_region']:
            ...
            constraints_tr(model, row['outcome'], data, clustering_model)
        if row['objective']!=0:
