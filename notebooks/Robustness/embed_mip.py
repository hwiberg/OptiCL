...
def optimization_MIP(model, model_master, max_violation_group = None):
    ...
    def constraints_linear(model, outcome, task, coefficients, var_features, contex_features, lb=None, ub=None, weight_objective=0, SCM=None, features=None):
        '''
    ...
    def constraints_tree(model, outcome, tree_table, var_features, contex_features, lb=None, ub=None, M=1e5, weight_objective=0, SCM=None, features=None):
        ...
        def constraintsTree_1(model, j):
            return sum(model.x[i]*coeff.loc[j, i] for i in var_features) + sum(contex_features[i]*coeff.loc[j, i] for i in contex_features.keys()) <= intercept.iloc[j] + M*(1-model.l[(outcome,str(l_ids.iloc[j]))])
    ...
    def constraints_rf(model, outcome, forest_table, var_features, contex_features, ub=None, lb=None, max_violation=None, weight_objective=0, SCM=None, features=None):
        ...
        for i, t in enumerate(T):
            ...
            constraints_tree(model, t, tree_table, var_features, contex_features, lb=None, ub=None, weight_objective=0, SCM=None, features=features)
    ...
    def constraints_mlp(model, data, outcome, task, weights, var_features, contex_features, lb=None, ub=None, weight_objective=0, SCM=None, features=None, M_l=-1e5, M_u=1e5):
        ...
        v_input = []  #[model.x[i] for i in features]
        for i in features:
            try:
                v_input.append(contex_features[i])
            except Exception:
                v_input.append(model.x[i])
    ...
    def constraints_tr(model, outcome, data, var_features, contex_features, clustering_model, enlarge):
        ...
        if not pd.isna(clustering_model):
            print('Using the clustering algorithm')
            assert enlarge[0] != 1, 'Enlargement not supported with clustering'
            ...
            def constraint_CTR1(model, i):
                if i in var_features:
                    return model.x[i] == sum(model.lam['TR_'+outcome, k] * data.loc[k, i] for k in samples)
                else:
                    contex_features[i] == sum(model.lam['TR_'+outcome, k] * data.loc[k, i] for k in samples)

            def constraint_CTR2(model, label):
                cluster = data[clustering_model.labels_ == label]
                cluster_samples = cluster.index
                return sum(model.lam['TR_'+outcome, k] for k in cluster_samples) == model.u[label]
            ...
            model.add_component(otcome+'ConstraintClusteredTrustRegion3', Constraint(rule=sum(model.u[label] for label in n_clusters) == 1))
        else:
            if enlarge[0] == 1:
                print('The l1 norm is used for the enlarged CH trust region')
                def constraint_ETR1(model, i):
                    if i in var_features:
                        return model.x[i] + model.e[i] == sum(model.lam['TR_'+outcome, k] * data.loc[k, i] for k in samples)
                    else: contex_features[i] + model.e[i] == sum(model.lam['TR_'+outcome, k] * data.loc[k, i] for k in samples)
                def constraint_ETR21(model, i):
                    return model.eHelp[i] >= model.e[i]
                def constraint_ETR22(model, i):
                    return model.eHelp[i] >= - model.e[i]
                model.add_component('constraint_ETR0', Constraint(rule=sum(model.lam['TR_'+outcome, k] for k in samples) == 1))
                ...
                def constraint_CTR1(model, i):
                    if i in var_features:
                        ...
                        return contex_features[i] == sum(model.lam['TR_'+outcome, k] * data.loc[k, i] for k in samples)
                model.add_component(outcome+'ConstraintClusteredTrustRegion1', Constraint(rule=sum(model.lam['TR_'+outcome, k] for k in samples) == 1))
                model.add_component(outcome+'ConstraintClusteredTrustRegion2', Constraint(data.columns, rule=constraint_CTR1))
    ...
    for o in outcomes:
        ...
        for i, row in model_master_sub.iterrows():
            ...
            if row['objective']!=0:
                ...
                print(f"Embedding constraints for {outcome}")
        ...
        if not grouped:
            ...
            if pd.isna(max_violation_group):
                ...
                if not pd.isna(ub):
                    def constraint_upperBoundViol(model, j):
                        return 1 / 1000 * (model.y[j] - ub) <= model.y_viol[(o, str(j))]
                    model.add_component('upperBoundViol'+o, Constraint(outcomes_sub, rule=constraint_upperBoundViol))
                if not pd.isna(lb):
                    def constraint_lowerBoundViol(model, j):
