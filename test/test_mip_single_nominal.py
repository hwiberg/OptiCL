...
def run_experiment_nominal(alg, bs, viol_rule, n_iterations = 100, fixed_seed = None):
    assert bs == 0, "Use run_experiment() instead"
    ...
    for alg in alg_list:
        if bs == 0:
            perf_list.append(pd.read_csv('results/%s/%s_%s_performance.csv' % (alg, version, outcome)))
        else:
            for idx in range(bs):
